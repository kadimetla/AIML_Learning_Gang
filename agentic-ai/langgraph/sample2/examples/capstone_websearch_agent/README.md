...
| [`05_full_memory.py`](05_full_memory.py) | Adds a `Store`-backed semantic cache (`check_memory` skips the whole pipeline if a question was already answered), a `Store`-backed procedural report-style rule `reporter` reads, and an explicit `get_state_history()` replay of one specific past HITL turn (episodic) alongside the working-memory `ResearchState` that was already there | [`../agent_memory/`](../agent_memory/) |
